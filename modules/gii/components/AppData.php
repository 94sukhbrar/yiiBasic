...
class AppData {
	...
	private function makeModels($override, $array_exclude) {
		...
		foreach ( $models as $model ) {
			...
			if ($override && in_array ( $modelClass, $array_exclude ))
				continue;
			$generator = new \app\modules\gii\generators\tumodel\Generator (); // \yii\gii\generators\model\Generator ();
	...
	private function makeCrud($model) {
		$generator = new \app\modules\gii\generators\tucrud\Generator (); // \yii\gii\generators\crud\Generator ();
		$generator->enableI18N = TRUE;
